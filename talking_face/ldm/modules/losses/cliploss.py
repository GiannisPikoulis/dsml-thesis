...
#         self.texture_loss = torch.nn.MSELoss()
    ...
    def clip_directional_loss(self, src_img: torch.Tensor, source_class: list, target_img: torch.Tensor, target_class: str) -> torch.Tensor:

#         if self.target_direction is None:
#             self.target_direction = self.compute_text_direction(source_class, target_class)
#             print(self.target_direction.shape)
        
        target_directions = []
        for src_label in source_class:
            target_direction = self.compute_text_direction(src_label, target_class)
            target_directions.append(target_direction)
        target_directions = torch.cat(target_directions, dim=0)
        ...
        return self.direction_loss(edit_direction, target_directions).mean()
