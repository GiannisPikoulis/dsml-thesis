...
if __name__ == '__main__':  
    ...
    test_dataset = AffectnetTest(size=128, 
    ...
    if os.path.exists('./samples/manipulations') and os.path.isdir('./samples/manipulations'):
        print("Directory samples/manipulations already exists")
    else:
        os.makedirs('./samples/manipulations', exist_ok=True)
        
    if os.path.exists(f'./samples/manipulations/{y_trg}') and os.path.isdir(f'./samples/manipulations/{y_trg}'):
        print(f"Directory samples/manipulations/{y_trg} already exists")
    else:
        os.makedirs(f'./samples/manipulations/{y_trg}', exist_ok=True)
    ...
    #             print(c_src.shape, c_trg.shape)

                z = model.encode_first_stage(img)
    #             print(z.shape)
                x_rec, x_lat, x0 = sampler.latent_manipulation(S=ddim_steps,
                                                            c_src=c_src,
                                                            c_trg=c_trg,
                                                            batch_size=bs,
                                                            x0=z,
    ...
    #            x_lat = model.decode_first_stage(x_lat)
                x0 = model.decode_first_stage(x0)
                x_rec = model.decode_first_stage(x_rec)
            
    #            x_lat = torch.clamp((x_lat+1.0)/2.0, min=0.0, max=1.0)
    ...
    np.save(f'samples/manipulations/{y_trg}/untuned_xrec_nsamples{len(test_dataset)}_ddim{ddim_steps}_eta{ddim_eta}_scale{scale}_strength{strength}_emotion={y_trg}_{cnfg}_{ckpt}.npy', all_x_rec)
    np.save(f'samples/manipulations/{y_trg}/untuned_xlat_nsamples{len(test_dataset)}_ddim{ddim_steps}_eta{ddim_eta}_scale{scale}_strength{strength}_emotion={y_trg}_{cnfg}_{ckpt}.npy', all_x_lat)
    #np.save(f'samples/manipulations/{y_trg}/untuned_x0_nsamples{len(test_dataset)}_ddim{ddim_steps}_eta{ddim_eta}_scale{scale}_strength{strength}_emotion={y_trg}_{cnfg}_{ckpt}.npy', all_x0)
    np.save(f'samples/manipulations/{y_trg}/untuned_fp_nsamples{len(test_dataset)}_ddim{ddim_steps}_eta{ddim_eta}_scale{scale}_strength{strength}_emotion={y_trg}_{cnfg}_{ckpt}.npy', all_file_paths)